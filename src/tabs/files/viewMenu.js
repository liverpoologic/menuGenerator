...
function RefreshViewMenu() {
  ...
  if (menuTitle === "Menu" || !menu) { return "no action required"; }
  ...
  menu.meals.forEach((meal, i) => {
    ...
    meal.recipes.forEach((menuRecipe, j) => {
      var recipe = Dict.recipes[menuRecipe.recipeTitle];
      ...
      else if (menuRecipe.morv === "sp") { recipeTitle.innerText = `${menuRecipe.recipeTitle} (${recipe.serves}) - ${menuRecipe.morv}: ${menuRecipe.specialCount}`; }
        ...
        if (menu.specials) {
          Object.keys(specialPeople).forEach(personName => {
            ...
            if (allergenList.length > 0) {
              specialsArr.push({ personName: personName, foodName: ingredient.foodName });
            }
          });
        }
