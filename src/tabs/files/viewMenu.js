...
module.exports = function(DATA) {
    ...
    function CreatePageEls(parentDiv) {
        ...
        els.viewMenuDiv = u
            .CreateEl('div')
            .style('width:400px; margin:30px 0 0 0')
            .parent(parentDiv)
            .end();
    }
    ...
    function RefreshMenu({ isPrint }) {
        ...
        menu.meals.forEach((meal, i) => {
            ...
            let mealTitle = u
                .CreateEl('h3')
                .className('viewMealTitle')
                .parent(parentDiv)
                .id(`${printPrefix}mealTitle${i}`)
                .innerText(`${day} ${meal.mealType}`)
                .end();
            if (isPrint && i === 0) {
                mealTitle.className = 'firstMealTitle';
            }

            if (Object.keys(meal.recipes).length > 0 && !isPrint) {
                // if there is a recipe in the meal AND we're not in print mode, print the recipes and hide/show buttons
            ...
            let mealDiv = u.CreateElement(
                'div',
                parentDiv,
                `${printPrefix}mealDiv${i}`,
                'mealDiv',
                '',
                displayOnLoad
            );
            let mCount = meal.modifier
                ? Number(menu.meateaters) + Number(meal.modifier.meateaters)
                : menu.meateaters;
            let vCount = meal.modifier
                ? Number(menu.vegetarians) + Number(meal.modifier.vegetarians)
                : menu.vegetarians;
            let mealNumbersText = `M: ${mCount} V: ${vCount}`;
            let mealNumbers = u
                .CreateEl('notes')
                .className('summary')
                .innerText(mealNumbersText)
                .parent(mealDiv)
                .end();
            ...
            meal.recipes.forEach((menuRecipe, j) => {
                ...
                let recipeWrapDiv = u
                    .CreateEl('div')
                    .parent(mealDiv)
                    .className(`wrapDiv ${recipe.recipeType}`)
                    .end();

                if (!recipe) {
                    u.CreateEl('h4')
                        .innerText(`Missing Recipe: ${menuRecipe.recipeTitle}`)
                        .parent(recipeWrapDiv)
                        .end();
                    //break out of this loop and move onto the next recipe
                    return;
                }
                let recipeTitle = u.CreateElement(
                    'h4',
                    recipeWrapDiv,
                    `${printPrefix}recipeTitle${i}${j}`,
                    '',
                    `${menuRecipe.recipeTitle} - ${menuRecipe.morv}`,
                    'inline-block'
                );
                if (menuRecipe.morv === 'b') {
                    recipeTitle.innerText = `${menuRecipe.recipeTitle}`;
                } else if (menuRecipe.morv === 'sp') {
                    let specialCount = menuRecipe.specialCount
                        ? menuRecipe.specialCount
                        : '';
                    recipeTitle.innerText = `${menuRecipe.recipeTitle} (${
                        menuRecipe.morv
                    }: ${specialCount})`;
                ...
                u.CreateEl('notes')
                    .parent(recipeDiv)
                    .innerText(`Original Recipe Serves ${recipe.serves}`)
                    .end();

                let ingredientTable = u
                    .CreateEl('table')
                    .parent(recipeDiv)
                    .id(`${printPrefix}ingredientTable${i}${j}`)
                    .className('recipeIngredientTable')
                    .end();
                ...
                recipe.ingredients.forEach((ingredient, k) => {
                        ...
                        Object.keys(specialPeople).forEach(personName => {
                            ...
                            if (allergens && dietaryReq.allergens) {
                                allergens.forEach(allergen => {
                                    if (
                                        dietaryReq.allergens.indexOf(
                                            allergen
                                        ) >= 0
                                    )
                                        allergenList.push(ingredient.foodName);
                                });
                            }
                            if (dietaryReq.foods) {
                                if (
                                    dietaryReq.foods.indexOf(
                                        ingredient.foodName
                                    ) >= 0
                                )
                                    allergenList.push(ingredient.foodName);
                    ...
                    if (menu.meateaters === null) {
                        ...
                        var qLarge = u.CalculateQLarge(
                            menuTitle,
                            i,
                            menuRecipe,
                            ingredient
                        );
                        html = u.DisplayIngredient(
                            ingredient.quantity,
                            qLarge,
                            food.unit
                        );
                    ...
                    u.CreateRow(
                        `${printPrefix}ingredientTable${i}${j}`,
                        'td',
                        html,
                        ids,
                        [50, 10, 10, 30],
                ...
                if (specialsArr.length > 0) {
                    ...
                    var specialsText = Object.keys(specialsObj2).map(
                        problemFoods => {
                            var people = specialsObj2[problemFoods];
                            return `${
                                people.length
                            } - ${problemFoods} (${people.join(', ')})`;
                        }
                    );
        ...
        //create right panel
        els.menuMorvCounts.innerText = `Meateaters: ${
            menu.meateaters
        }    Vegetarians: ${menu.vegetarians}`;
        console.log(menu);
    }
        ...
        menu.meals.forEach((meal, i) => {
            ...
            meal.recipes.forEach((recipe, j) => {
                ...
                let titleText =
                    recipe.morv == 'b'
                        ? recipe.recipeTitle
                        : `${recipe.recipeTitle} - ${recipe.morv}`;
                let recipeTitle = u
                    .CreateEl('h4')
                    .parent(wrapDiv)
                    .className(actual_recipe.recipeType)
                    .innerHTML(titleText)
                    .end();

                let allRecipeNotes = u
                    .ID(`recipeDiv${i}${j}`)
                    .getElementsByClassName('summary');
                let recipeNotesText = Array.prototype.map
                    .call(allRecipeNotes, x => x.innerHTML)
                    .join('<br>');

                let recipeNotesEl = u
                    .CreateEl('notes')
                    .parent(wrapDiv)
                    .innerHTML(recipeNotesText)
                    .end();
            });
        });
    }
