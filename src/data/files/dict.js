    ...
    addAllergens(thing, allergens) {
        ...
        if (allergens.length === 1 && allergens[0].length === 0) {
            ...
            allergens.forEach(allergen => {
                if (
                    Config.enums.allergenEnum.indexOf(allergen) < 0 &&
                    allergen !== ''
                ) {
...
Dict.recipes = {
    addRecipe(obj) {
        ...
        if (typeof obj.recipeTitle != 'string') {
            ...
            console.log(
                `invalid input: ${obj.mealType} not in the mealType enum`
            );
        } else if (Config.enums.recipeTypeEnum.indexOf(obj.recipeType) < 0) {
            console.log(
                `invalid input: ${obj.recipeType} not in the recipeType enum`
            );
...
Dict.menus = {
        ...
        if (typeof menuTitle != 'string') {
            ...
            if (this[menuTitle].meals.length === 0) {
                ...
                for (let i = 0; i < this[menuTitle].meals.length; i++) {
                    let compare = CompareMeal(
                        newMeal,
                        this[menuTitle].meals[i]
                    );
                    if (compare === 0) {
                        console.log(
                            'tried to add repeated meal. Meal not added.'
                        );
                        return;
                    } else if (
                        compare === 1 &&
                        i === this[menuTitle].meals.length - 1
                    ) {
                        // check if the meal needs to go at the end of the array
        ...
        if (typeof menuTitle != 'string') {
            ...
            meal.recipes.forEach(recipe => {
                if (recipe.recipeTitle === recipeTitle) {
                    console.log(
                        'tried to add repeated recipe. Recipe not added.'
                    );
                    return 'error';
                }
            });
            ...
            if (meal.recipes.length === 0) {
                // if there are no existing recipes. newRecipe is recipe[0]
                ...
                for (let i = 0; i < meal.recipes.length; i++) {
                    ...
                    if (compare === 1 && i === meal.recipes.length - 1) {
                        // check if the recipe needs to go at the end of the array
    ...
    changeAllergyType(menuTitle, personName, allergens, foods, morv) {
        console.log('changing allergy type');
        ...
        Config.enums.specialsEnum[personName] = this[menuTitle].specials[
            personName
        ];
    },
...
Dict.read = function(backupFlag) {
    ...
    var fileName = remote.getGlobal('sharedObject').fileNames[coreOrBackup]
        .dict;
...
function CompareRecipe(recipeA, recipeB) {
    ...
    if (recipeA.morv === 'sp' && recipeB.morv !== 'sp') {
        ...
        e.recipeTypeEnum.indexOf(a.recipeType) >
        e.recipeTypeEnum.indexOf(b.recipeType)
    ) {
        return 1;
    } else if (
        e.recipeTypeEnum.indexOf(a.recipeType) <
        ...
        e.recipeMorv.indexOf(recipeA.morv) > e.recipeMorv.indexOf(recipeB.morv)
    ) {
        return 1;
    } else if (
        e.recipeMorv.indexOf(recipeA.morv) < e.recipeMorv.indexOf(recipeB.morv)
...
function CompareMeal(a, b) {
    ...
    if (aDate > bDate) {
        ...
        e.mealTypeEnum.indexOf(a.mealType) > e.mealTypeEnum.indexOf(b.mealType)
    ) {
        comparison = 1;
    } else if (
        e.mealTypeEnum.indexOf(a.mealType) < e.mealTypeEnum.indexOf(b.mealType)
