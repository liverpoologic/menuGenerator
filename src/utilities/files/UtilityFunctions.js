...
module.exports = function(DATA) {
   ...
   function AddCells(numberOfCells, parentRow, cellType) {
      for (let i = 0; i < numberOfCells; i++) {
         CreateEl(cellType).parent(parentRow).end()
      }
   }
   ...
   function CreateModalFramework(modalId, title, editableTitleKey) {
      let ourEls = {};
      ourEls.modal = CreateEl('div').id(modalId).className('modal').parent(ID('modals')).end();
      ourEls.modalContent = CreateEl('div').className('modal-content animate').parent(ourEls.modal).end();
      ourEls.titleContainer = CreateEl('div').className('container').style('padding-bottom:0px').parent(ourEls.modalContent).end();
      ourEls.title = CreateEl('text').className('modal-title').innerText(`${title}: `).parent(ourEls.titleContainer).end();
      ourEls[editableTitleKey] = CreateEl('text').className('modal-title').parent(ourEls.titleContainer).end();
      ourEls.content = CreateEl('div').className('container').parent(ourEls.modalContent).end();
   ...
   return {
      ...
      CreateModalFramework: CreateModalFramework,
