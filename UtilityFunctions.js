...
function CompareFoodType(a, b) {
  ...
  if (typeof Dict.foods[a] === "undefined") { console.log(`${a} needs to be added to the food dictionary`); return "error, view console"; }
  if (typeof Dict.foods[b] === "undefined") { console.log(`${b} needs to be added to the food dictionary`); return "error, view console"; }
...
function Html(variable, id, className, style, innerHTML, innerText, value) {
  if (variable === "") {
    ...
    if (id !== undefined && id !== "") { variable.id = id; }
...
function WriteConfig() {
  ...
  fs.writeFileSync(`./resources/${fileName}`, JSON.stringify(Config), { encoding: "utf8" });
...
function WriteDict(dictID) {
  ...
  fs.writeFileSync(`./resources/${fileName}`, JSON.stringify(Dict), { encoding: "utf8" });
...
function SaveBackup() {
  ...
  fs.writeFileSync(`./resources/${dictFileName}`, JSON.stringify(Dict), { encoding: "utf8" });

  var configFileName = remote.getGlobal('sharedObject').fileNames.backup.config;
  fs.writeFileSync(`./resources/${configFileName}`, JSON.stringify(Config), { encoding: "utf8" });
